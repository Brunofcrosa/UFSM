...
void selectionSort(float vetor[], int tamanho, int *pComparacoes, int *pTrocas) {
    int i, j, indiceMin;
    *pComparacoes = 0;
    *pTrocas = 0;
    for (i = 0; i < tamanho - 1; i++) {
        ...
        if (indiceMin != i) {
            trocar(&vetor[i], &vetor[indiceMin]);
            (*pTrocas)++;
        }
    }
}
...
int main() {
    ...
    double tempoCPU;
    inicio = clock();
    selectionSort(precos, tamanho, &numComparacoes, &numTrocas);
    fim = clock();
